...
COMMENT ON FUNCTION public.change_app_state(p_app uuid, p_ziel public.lifecycle_state, p_actor uuid) IS 'K01-M28 · O-K01-20 · Zeichnung vom 05.08.2026. Der einzige Weg zu einem Zustandswechsel. Die Uebergangsregeln bleiben bei state_transition und lifecycle_transition_guard -- diese Funktion kanalisiert nur den Zugang.';


--
-- Name: create_app_after_fit(uuid, text, text, uuid, uuid); Type: FUNCTION; Schema: public; Owner: frxadmin
--

CREATE FUNCTION public.create_app_after_fit(p_tenant uuid, p_project_no text, p_name text, p_fit_check uuid, p_actor uuid) RETURNS uuid
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'pg_temp'
    AS $$
...
ALTER FUNCTION public.create_app_after_fit(p_tenant uuid, p_project_no text, p_name text, p_fit_check uuid, p_actor uuid) OWNER TO frxadmin;

--
-- Name: FUNCTION create_app_after_fit(p_tenant uuid, p_project_no text, p_name text, p_fit_check uuid, p_actor uuid); Type: COMMENT; Schema: public; Owner: frxadmin
--

COMMENT ON FUNCTION public.create_app_after_fit(p_tenant uuid, p_project_no text, p_name text, p_fit_check uuid, p_actor uuid) IS 'K01-M27 · O-K01-20 · Zeichnung vom 05.08.2026. Der einzige Weg zu einer app-Zeile. fr_portal hat kein INSERT auf app -- wer die Funktion umgeht, kommt nicht an der Tabelle an, sondern an einer fehlenden Berechtigung.';
...
--
-- Name: FUNCTION create_app_after_fit(p_tenant uuid, p_project_no text, p_name text, p_fit_check uuid, p_actor uuid); Type: ACL; Schema: public; Owner: frxadmin
--

GRANT ALL ON FUNCTION public.create_app_after_fit(p_tenant uuid, p_project_no text, p_name text, p_fit_check uuid, p_actor uuid) TO fr_portal;
